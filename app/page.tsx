...
export default function Home() {
  ...
  const filteredOutlets = useMemo(() => {
    ...
    return filtered;
  }, [selectedCountry, biasFilter, sortBy, mediaTypeFilter]);
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="w-full overflow-x-auto sm:w-auto">
              <Tabs value={selectedCountry} onValueChange={setSelectedCountry}>
                <TabsList className="flex h-8 flex-nowrap justify-start gap-1 bg-transparent p-0 sm:flex-wrap">
                  <TabsTrigger value="all" className="h-7 shrink-0 px-2.5 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    All
                  </TabsTrigger>
                  {COUNTRIES.map(country => (
                    <TabsTrigger key={country} value={country} className="h-7 shrink-0 px-2.5 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                      {country}
                    </TabsTrigger>
                  ))}
                </TabsList>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {countryData['far-left'].length > 0 ? (
                        countryData['far-left'].map(outlet => (
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {countryData['far-right'].length > 0 ? (
                        countryData['far-right'].map(outlet => (
                          <MediaOutletCard key={outlet.id} outlet={outlet} />
                        ))
                      ) : (
                        <Card className="p-4">
        ...
        <div className="mt-12 text-center">
          <p className="text-xs text-muted-foreground sm:text-sm">
            Last updated: January 2025 • Data sources: Multiple independent fact-checkers and media watchdog organizations
          </p>
        </div>
      </div>
    </main>
