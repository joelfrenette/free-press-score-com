...
import { ArrowLeft, TrendingUp, AlertCircle, FileText, Scale, Flag, CheckCircle, XCircle, Clock, DollarSign, Building2, Eye } from 'lucide-react';
    ...
    <main className="min-h-screen bg-background">
      <div className="container mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground">{outlet.description}</p>
        </div>
        ...
        <div className="mb-8 grid gap-6 md:grid-cols-3">
          <Card className="p-6">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-foreground">Fact-Check Accuracy</h3>
            </div>
            <div className="mb-2 flex items-end gap-2">
              <span className={`text-3xl font-bold ${getScoreColor(outlet.factCheckAccuracy)}`}>
          ...
          <Card className="p-6">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-foreground">Editorial Independence</h3>
            </div>
            <div className="mb-2 flex items-end gap-2">
              <span className={`text-3xl font-bold ${getScoreColor(outlet.editorialIndependence)}`}>
          ...
          <Card className="p-6">
            ...
            <div className="mb-2 flex items-end gap-2">
              <span className={`text-3xl font-bold ${getScoreColor(outlet.transparency)}`}>
                {outlet.transparency}
              </span>
              <span className="mb-1 text-sm text-muted-foreground">/ 100</span>
            </div>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <Badge 
                            ...
                            className={`shrink-0 text-xs ${
                              stakeholder.politicalLean === 'far-left' ? 'bg-[var(--far-left)]/20 text-[var(--far-left)]' :
                              stakeholder.politicalLean === 'left' ? 'bg-[var(--left-bias)]/20 text-[var(--left-bias)]' :
                              stakeholder.politicalLean === 'center' ? 'bg-[var(--center-bias)]/20 text-[var(--center-bias)]' :
                              stakeholder.politicalLean === 'right' ? 'bg-[var(--right-bias)]/20 text-[var(--right-bias)]' :
                              stakeholder.politicalLean === 'far-right' ? 'bg-[var(--far-right)]/20 text-[var(--far-right)]' :
                              ''
                            }`}
                          >
                            {stakeholder.politicalLean === 'unknown' ? 'Unknown' : stakeholder.politicalLean.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                          </Badge>
                        </div>
                      </div>
                    ))}
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <Badge 
                            variant="secondary"
                            className={`shrink-0 text-xs ${
                              member.politicalLean === 'far-left' ? 'bg-[var(--far-left)]/20 text-[var(--far-left)]' :
                              member.politicalLean === 'left' ? 'bg-[var(--left-bias)]/20 text-[var(--left-bias)]' :
                              member.politicalLean === 'center' ? 'bg-[var(--center-bias)]/20 text-[var(--center-bias)]' :
                              member.politicalLean === 'right' ? 'bg-[var(--right-bias)]/20 text-[var(--right-bias)]' :
                              member.politicalLean === 'far-right' ? 'bg-[var(--far-right)]/20 text-[var(--far-right)]' :
                              ''
                            }`}
                          >
                            {member.politicalLean === 'unknown' ? 'Unknown' : member.politicalLean.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                          </Badge>
                        </div>
                      </div>
                    ))}
        ...
        {(outlet.retractions.length > 0 ||
          ...
          <Card className="mb-8 p-6">
            ...
            <div className="mb-6 flex flex-wrap gap-4 text-sm text-muted-foreground">
              <span>
                <strong className="text-foreground">{outlet.retractions.length}</strong> Corrections/Retractions
              </span>
              <span>•</span>
              <span>
                <strong className="text-foreground">{outlet.lawsuits.length}</strong> Legal Cases
              </span>
              <span>•</span>
              <span>
                <strong className="text-foreground">{outlet.scandals.length}</strong> Controversies
              </span>
            </div>
        ...
        <Card className="border-accent/50 bg-accent/5 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Our scores are derived from multiple independent fact-checking organizations, media
            watchdog groups, and transparency reports. Bias scores reflect editorial positioning
            based on content analysis. For more information about our methodology,{' '}
