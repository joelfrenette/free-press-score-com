...
export function MediaOutletCard({ outlet }: MediaOutletCardProps) {
  ...
  return (
    <Link href={`/outlet/${outlet.id}`}>
      <Card
        className={`group relative overflow-hidden transition-all hover:shadow-lg hover:scale-[1.02] ${getBiasBgColor(outlet.biasScore)} border`}
      >
        <div className="p-4">
          ...
          <h3 className="mb-1 font-semibold text-foreground group-hover:text-accent">{outlet.name}</h3>
          ...
          <div className="mt-2 flex items-center gap-1">
            ...
            <span className="text-xs font-semibold text-foreground">{outlet.metrics?.avgMonthlyAudience ?? "N/A"}</span>
          </div>

          <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{outlet.description}</p>
        </div>
      </Card>
    </Link>
  )
